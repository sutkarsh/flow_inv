# From https://github.com/tychovdo/lila/blob/main/lila/datasets.py
...
# Based on the implementation in https://github.com/AllanYangZhou/generative-invariance-transfer
class CIFAR10LT(datasets.CIFAR10):
    """ CIFAR10 Long Tail (rho = 10) """

    def __init__(self, root: str, train: bool = True, transform: Union[Callable, type(None)] = None,
                 target_transform: Union[Callable, type(None)] = None, download: bool = False):
        """
        Args:
            root (string): Root directory of dataset where ``CIFAR10/processed/training.pt``
        ...
        torch.manual_seed(int(train))
        self.gen_imbalanced_data(train)


    def get_img_num_per_cls(self, cls_num=10, imb_type='exp', imb_factor=0.1):
    ...
    def gen_imbalanced_data(self, train):
        if not train:
            ...
            logging.info(f"img_num_per_cls: {img_num_per_cls}")
            ...
            self.target = targets_np[indices]
