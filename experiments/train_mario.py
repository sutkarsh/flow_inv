...
def make_polar_plot(ax, model, orig_inputs, args : Args, ang=None, tag="polar_plot"):
    ...
    for a, c in zip(ang, colors):
        ...
        ax.plot((np.array(np.array(rots[1:-2])+a)*np.pi/180),0.9*hist[1:-1]/hist[1:-1].max(), label="Inv for one example", c=c)
    for a, c in zip(ang, colors):
...
def make_rot_plot(ax, model, orig_inputs, lab=None):
    ...
    ax.plot(rots[:-1], hist/hist.max(), label="Inv for one example", c='blue')
    
    loss_vals = []
    # Also adding a (normalized) loss plot in the same range
    for ang in np.linspace(-180, 180, 100):
    ...
    ax.plot(np.linspace(-180, 180, 100), loss_vals/loss_vals.max(), c='red', marker='o', label='loss')
...
def trainer(model, trainloader, testloader, args: Args):
    ...
    for epoch in tqdm.trange(args.optimizer.epochs, position=0, leave=False):  # loop over the dataset multiple times
        for i, data in tqdm.tqdm(enumerate(trainloader), position=1, leave=False, total=len(trainloader)):
            ...
            if args.trainer.zero_mean_reg:
                emb = model.pose_embedder(inputs)
                inputs = inputs.repeat(n_copies, 1, 1, 1)
            ...
            loss = classification_loss + aug_loss_factor * average_logp.mean() + args.trainer.zero_mean_reg * (weights**2).mean()
            loss.backward()
        ...
        print("length", len(plot_dict['crossentloss']))
...
def main(args : Args):
    setup_logger(LOG, "../logs/exp_mario/")
    ...
    if args.method == method_choice.NFAUG:
        ...
        augmenter = InstaAug(aug_type=args.model.aug_type)
    else:
        raise NotImplementedError("Invalid method")

    model = align_model_synch(in_channel = in_channel, mp = mp,
                        pose_emb_dimension=pose_emb_dimension, pose_emb_netwidth=8,
